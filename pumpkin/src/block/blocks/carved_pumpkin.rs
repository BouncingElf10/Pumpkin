...
async fn spawn_golem(world: &Arc<World>, pattern: GolemPattern) {
    ...
    let golem: Arc<dyn crate::entity::EntityBase> =
        if pattern.entity_type == &EntityType::SNOW_GOLEM {
    ...
    fn placed<'a>(&'a self, args: PlacedArgs<'a>) -> BlockFuture<'a, ()> {
        Box::pin(async {
            if let Some(pattern) = find_golem_pattern(args.world, args.position) {
                spawn_golem(args.world, pattern).await;
            }
        })
    }
}
